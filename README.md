...
- The page's background is matched to aethra.co.in's own: the same flat `#dceaf9` plus its exact fixed, 0.05-opacity SVG fractal-noise grain overlay (same data URI, inspected directly from the live site's computed styles rather than approximated). Every other color token was then re-derived as a genuine light-theme palette against that specific background (text, borders, bubbles, badges) rather than just dropping a light background behind the old dark-theme text - the original dark palette's near-white text on this background would have been unreadable.
