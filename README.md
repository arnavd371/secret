...
- Real LaTeX rendering (KaTeX, pinned version with SRI hashes computed from the actual served files, not blind CDN trust) on every response bubble: the `\( \)`/`\[ \]` delimiters the system prompt actually instructs the model to use, plus `$ $`/`$$ $$` as a fallback for the convention models default to regardless. Applied via KaTeX's auto-render walking the already-`textContent`-populated DOM, never `innerHTML` - a draft that happened to contain HTML-looking text stays exactly as inert as it already was, this only adds math typesetting on top.
