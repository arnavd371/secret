...
- An in-app "How to use this tutor" guide: a real six-step walkthrough (ask about a topic, get unstuck with a hint, get work checked, photograph work instead of typing it, ask for a harder problem, progress carries over via the student/session codes), shown automatically on a visitor's first load and reopenable anytime via the "?" button - `localStorage` remembers it's been seen so it doesn't nag on every visit.
- Real LaTeX rendering (KaTeX, pinned version with SRI hashes computed from the actual served files, not blind CDN trust) on every response bubble: the `\( \)`/`\[ \]` delimiters the system prompt actually instructs the model to use, plus `$ $`/`$$ $$` as a fallback for the convention models default to regardless.
- Real Markdown rendering too (`marked` + `DOMPurify`, both pinned with real SRI hashes): the system prompt tells the Tutor to output Markdown (`## Header`, `**bold**`, numbered lists), and it was showing up as literal syntax with nothing to render it. Content is now set via sanitized `innerHTML` - a real change from the earlier textContent-only approach, with `DOMPurify.sanitize()` as the actual safety boundary rather than "never innerHTML at all" (verified live: an injected `<script>`/`onerror` payload rendered completely inert, confirmed via a real page-level check that neither ran). Math is protected from Markdown's own escape handling before either renders - `marked.parse()` silently strips the backslash from `\(a_{ij}\)` (a real bug caught before shipping, not a hypothetical one), so every math segment is pulled into an opaque placeholder, Markdown runs on text with no delimiters left to mangle, and the real math source is swapped back in as plain text immediately before KaTeX's own pass typesets it.
- The page's background is aethra.co.in's own animated one, not an approximation: aethra.co.in/bg.js was fetched and read directly (a real, unminified Three.js scene) - a simplex-noise-displaced plane rendered through a custom Bayer-dithered ink shader, producing the same slowly-shifting cloud/wave texture live at `#dceaf9` paper / `#3f5170` ink, ported into this page verbatim (same shader source, same colors, same mount convention) with only a top-level try/catch added so a WebGL failure degrades to the flat paper color instead of breaking the page. The separate static SVG grain overlay (`#ink-noise`) sits on top of it, matching the reference site's own two-layer structure. Every other color token (text, borders, bubbles, badges) was re-derived as a genuine light-theme palette against that background rather than just dropping a light background behind the old dark-theme text - the original near-white text would have been unreadable on it.
...
- Two real bugs only a live model call could surface, caught by actually using the deployed app, not just by testing against mocks: the Tutor's system prompt (`app/agents/templates.py`) carried an "OUTPUT SCHEMA" JSON instruction from the spec's own §7.7 skeleton that nothing downstream ever parsed - a weaker model complying with it literally leaked a stray `{"text": ..., "citations": ..., "ui_hints": ...}` blob into what the student saw. And the no-grounding fallback text literally suggested the model say "you can't confirm the IB-specific convention" - turning a real safety property (don't assert unverified syllabus facts) into visible meta-commentary ("without specific curriculum context, I can only provide...") on every ungrounded answer. Both fixed at the source (the prompt no longer asks for JSON or invites narrating its own limitations) and backstopped the same way every other Tutor-agent risk already is: `tutor_agent._strip_trailing_json_echo` deterministically strips a real trailing JSON echo (verified by a proper brace-balance walk, not a greedy regex, so legitimate LaTeX like `\frac{d}{dx}` is never touched) regardless of whether the prompt succeeds at preventing it.
